...
The data set used in these exercises was compiled from data downloaded from the website of the UK's national weather service, the [*Met Office*](http://www.metoffice.gov.uk/public/weather/climate-historic/). It is saved in the file **climate.xlsx**[^1] which can be found in the folder `Data/`. The spreadsheet contains monthly data from five UK weather stations for the following variables:
...
2.  **Load data**. Start by importing the dataset with the `Import Dataset` button and name it `climate`. When you do this, a command starting with `read_excel()` is shown in the console. It is a good idea to **copy the command into your script** so that the next time you run your script you can just execute that line instead of having to find the file again. But make sure to use full path or relative whre your Quarto document is saved.

::: {.callout-note collapse="true"}
## What is the character in the `read_excel()` function?
...
The **Full** path could look like the one below, where the directions are separated by slashs `/`. This means that we start in the `Desktop` folder, then go to the `Work`, to the `Courses` folder, to the `FromExceltoR` folder, to the `Data` folder where we select the `climate.xlsx` file.

```{r eval=FALSE}
climate <- read_excel("Desktop/Work/Courses/FromExceltoR/Data/climate.xlsx")
```
...
![](../Figures/file_tree_absolute_path.png){fig-align="center"}

Or you can use relative path. If your Quarto working directory is set to `Scripts/`, then you would need go out one step and go back into Data folder:
...
7.  Filter/select from the climate dataset (remember to `filter` rows and `select` columns):
...
15. How many months are there with good weather (use the column you made in 14) for each station? Find the station that has the most months with good weather.
    ...
    **Report the value here.**

    ![](images/qr_code_Ex2Q1.png){width="200"}

## Complex operations

The final questions require that you combine commands and variables of the type above.

16. For each weather station apart from the one in Armagh, compute the total rainfall and sunshine duration for months that had no days of air frost. Present the totals in centimeters and days, respectively.
...
19. Modify the station names so that they start with capital letters.

20. Create a new column that contains the month names (e.g., 'January', 'February', etc.). Could someone before you have needed the `month.name` in R?

21. Create a new column that assigns a season (Winter, Spring, Summer, Fall) based on the month.
...
26. Extract the unique stations and save them as a vector.
...
27. Count how many times the letter 'A' appears in each station.

    **Let us know you’ve mastered the tidyverse, and report the value here.**

    ![](images/qr_code_Ex2Q2.png){width="200"}

28. Count the number of both uppercase 'A' and lowercase 'a' in each station.

29. In the climate change data frame, create a new column for each word in the `device` column. For example, you can use the `str_split_i()` function to extract individual words and assign them to separate columns.

30. Modify one of the new columns to contain only lowercase letters and the other to contain only uppercase letters.

31. Add a new columns that contains the first three letters of each month in uppercase.
