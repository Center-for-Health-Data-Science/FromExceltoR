...
An R package needs be *installed* and *loaded* before you can use its functionalities. You only have to install a package once (until you re-install R), whereas you have to load it in every R session you want to use it. As an example, let's install the package `cowplot`.
...
11. **Loading**. Load the package you just installed with the command `library(cowplot)`. If everything went well, you should now able to run the command `data(package = 'cowplot')` which will show you an overview of the datasets included in the package.
...
17. Display the volume of the first 10 trees.
