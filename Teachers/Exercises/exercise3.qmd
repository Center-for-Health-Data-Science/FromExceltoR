...
The `geom_line` function needs the `group` aesthetic to stratify categories.
...
12. The **month** variable was read into R as a numerical variable. Convert this variable to a factor and make the line plot from 11 again. What has changed?
...
14. Add `geom_point()` to the line plot from above.
...
23. Make a histogram of rain from the climate dataset (find the correct `geom_` for this). Interpret the plot, what does it show?
...
26. Make a bar chart (`geom_col()`) which visualizes the sunshine hours per month. If you have not done so in question 12, convert month to a factor now and remake the plot.
...
32. Add labels to each bar that state the sum of the rainfall per station. You can do this by adding `geom_label()` to the plot and providing the information under the `label` keyword inside the `aes()`.
...
35. Load in the climate change dataset you exported in the last optional exercises.

36. Use `group_by`, `summarize`, and `facet_wrap` to recreate this plot. Consider if you need to change the class of some of the variables prior to plotting. ![](../Figures/exercise3_36.png)

37. Your supervisor does not like colors. Change the stratification to be based on `linetype` instead of `color`. Add a white background, update the labels to start with uppercase letters, and provide the plot with a meaningful title.

38. Recreate this plot. Use whatever colors you like (but change them from the default coloring) and give the plot at meaningful title. ![](../Figures/exercise3_38.png)

39. Recreate this plot using `geom_tile()` to make a heatmap, and `scale_fill_gradient2()` to select custom colors. ![](../Figures/exercise3_39.png)
