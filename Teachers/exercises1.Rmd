...
An R package needs be _installed_ and _loaded_ before you can use its functionalities. You only have to download and install a package once (until you re-install R), whereas you have to load it in every R session you want to use it. As an example, let's install the package `emmeans`.
...
b.  Using the graphical interface:
    ...
    Look at lower right of your Rstudio window where you have a window with several tabs. Click on _Packages_. You will see a list of your currently installed packages and their versions. To install `emmeans`, click on _Install_ and start to type the name. You will notice a drop down list appears from which you can select the correct package. This is useful if you are not quite sure of the correct spelling.
...
7.  Loading: Load the package you just installed with the command `library(emmeans)`. If everything went well, you should now able to run the command `oranges`. This shows a dataset which in included in the package.
...
19. Locate the R script that you saved in question 5; that would be the file *solution1.R* if you followed our name suggestion in question 5. Start RStudio again, and open the file (via the File menu). Check that you can run it again. 
