...
We will use **ggplot2** to make plots, and to be prepared for data manipulations, we simply load this together with the rest of the **tidyverse**. We will also use helper functions from **GGally** and **ggfortify** packages.
...
You will focus on the gene **IGFL4**, part of the insulin-like growth factor family involved in energy metabolism, growth, and development.

The data are stored in `psoriasis.xlsx`.
...
To get an impression of the data, we make a violin plot of IGFL4 expression levels from each skin sample (healthy, psne, psor), and compute group-wise means and standard deviations.
...
Before we decide on statistical test to use to compare our groups, lets check if our gene expression data is normal distributed. If data are not normal, consider transformations (log, square root) or non-parametric tests (Wilcoxon, Kruskal–Wallis).

Testing for Normality in R is done by Visual inspection or statistical test.

We will use a nice helper function from ggpubr package to plot a Quantile-Quantile plot.

```{r}
library(ggpubr)
# nice ggplot2-based QQ plot
ggqqplot(psorData$IGFL4)  # nice ggplot2-based QQ plot

```

And Shapiro–Wilk to confirm statistically.
...
Where Null hypothesis (H₀): data are normally distributed.
...
We will use a **one-way ANOVA** to test whether IGFL4 expression differs across the three groups. In R, you can fit a one-way ANOVA with the `lm()` function. It’s good practice to save the fitted model object (e.g., `oneway`) so you can reuse it for further analysis.
...
OR we can use a fancy function from **ggfortify** package to build beautifull ggplot diagnostic plots for linear models.
...
In this case there appears to be problems with the model. In particular, the upper left plot shows a quadratic tendency suggesting that the linearity assumption is not appropriate. Or we have an outlyer. Perhaps a data transformation can help us out.
...
#### Step 5: Hypothesis test + Report of the model
