...
## GO & Pathway enrichment analysis with KEGG

For gene ontology enrichment and pathway enrichment analysis we will use the R-package `clusterProfiler` <http://yulab-smu.top/clusterProfiler-book/index.html> and the `KEGG` ontologies and pathways.   

First, we check that we have pathway information for our species of interest, in this case mouse:

```{r}
search_kegg_organism('mmu', by='kegg_code')
```

Pathway enrichment analysis, using all genes from our expression dataset as the gene universe (e.g. background for enrichment):
...
Gene ontology enrichment analysis, using all genes from our expression dataset as the gene universe (e.g. background for enrichment). 
Here we enrich for _molecular function_ (MF), but there are also other options for type of ontology. 
Other parameters set are: Benjamini holbech correction for multiple testing and cutoffs for significance.
...
Below we perform the same steps as above to get the DE genes between (I) pregnant and control mice and (II) lactating and pregnant mice:  

(I) pregnant and control mice:
```{r, eval=FALSE}
resPCpw <- enrichKEGG(gene = resPC$EntrezGeneID,
                      organism = 'mmu', 
                      universe=unique(GeneNames$EntrezGeneID))
head(resPCpw)
```
