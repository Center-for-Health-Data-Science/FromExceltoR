...
```{r}
downloads3 <- filter(downloads, size > 0)
downloads3
```
...
filter(downloads3, machineName == "kermit", size > 200000)
...
downloads4 <- select(downloads3, machineName, size, time)
downloads4
```
...
```{r}
downloads4 <- mutate(downloads4, speed = size / time, logSize = log10(size))
downloads4
```
...
```{r}
downloads4 <- mutate(downloads4, slow = ifelse(speed < 150, "Yes", "No"))
downloads4
```
